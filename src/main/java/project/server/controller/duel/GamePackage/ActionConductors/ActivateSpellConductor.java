...
public class ActivateSpellConductor {
    ...
    private static void returnAllSpellTrapsToRespectiveHands(String token, Action uninterruptedAction) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (card != null && !(uninterruptedAction.getFinalMainCardLocation().getIndex() == i + 1 &&
                uninterruptedAction.getFinalMainCardLocation().getRowOfCardLocation().equals(RowOfCardLocation.ALLY_SPELL_ZONE))) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            card = duelBoard.getCardByCardLocation(cardLocation);
            if (card != null && !(uninterruptedAction.getFinalMainCardLocation().getIndex() == i + 1 &&
                uninterruptedAction.getFinalMainCardLocation().getRowOfCardLocation().equals(RowOfCardLocation.OPPONENT_SPELL_ZONE))) {
                SendCardToGraveyardConductor.removeCardAndGetRemovedCard(cardLocation, token);
