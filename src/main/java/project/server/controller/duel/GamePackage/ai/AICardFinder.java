...
public class AICardFinder {
    ...
    protected String findCardRemovalSpellCardInHandToActivate(AI ai) {
        ArrayList<Card> aiCardsInHand = ai.getAiBoardUnderstander().getAiCardsInHand();
        for (int i = 0; i < aiCardsInHand.size(); i++) {
            if (Card.isCardASpell(aiCardsInHand.get(i))) {
                SpellCard spellCard = (SpellCard) aiCardsInHand.get(i);
    ...
    protected String findSpecialSummoningMonsterCardInHandToActivate(AI ai) {
        ArrayList<Card> aiCardsInHand = ai.getAiBoardUnderstander().getAiCardsInHand();
        for (int i = 0; i < aiCardsInHand.size(); i++) {
            if (Card.isCardAMonster(aiCardsInHand.get(i))) {
                MonsterCard monsterCard = (MonsterCard) aiCardsInHand.get(i);
    ...
    protected String findSpecialSummoningSpellCardsToActivate(AI ai) {
        ArrayList<Card> aiCardsInHand = ai.getAiBoardUnderstander().getAiCardsInHand();
        for (int i = 0; i < aiCardsInHand.size(); i++) {
            if (Card.isCardASpell(aiCardsInHand.get(i))) {
                SpellCard spellCard = (SpellCard) aiCardsInHand.get(i);
                ...
                if (normalSpellCardEffects.contains(NormalSpellCardEffect.SPECIAL_SUMMON_MONSTER_FROM_EITHER_GY)) {
                    ai.setShouldRedirectAIMind(true);
                    ai.setAiActionType(AIActionType.ACTIVATE_EFFECT);
                    ai.setAiFurtherActivationInput(AIFurtherActivationInput.SELECT_A_MONSTER_WITH_HIGH_ATTACK_FROM_EITHER_GRAVEYARD);
                    return "select --hand " + (i + 1);
                }
            }
        }
    ...
    protected String findTributeMonsterToSummon(AI ai) {
        ArrayList<Card> aiCardsInHand = ai.getAiBoardUnderstander().getAiCardsInHand();
        for (int i = 0; i < aiCardsInHand.size(); i++) {
            if (Card.isCardAMonster(aiCardsInHand.get(i))) {
                MonsterCard monsterCard = (MonsterCard) aiCardsInHand.get(i);
                if (monsterCard.getAttackPower() == 2400 && monsterCard.getCardName().contains("Caius")) {
                    ai.setShouldRedirectAIMind(true);
                    ai.setAiActionType(AIActionType.NORMAL_SUMMON);
                    return "select --hand " + (i + 1);
                }
