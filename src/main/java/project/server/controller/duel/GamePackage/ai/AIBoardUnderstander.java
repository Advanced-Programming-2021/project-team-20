...
public class AIBoardUnderstander {
    ...
    private void updateInformationAboutMonsterCardsInThisArrayList(int i, MonsterCard monsterCard, AIBoardAnalysis aiBoardAnalysis, String token) {
        if (monsterCard != null) {
            ...
            if (summoningRequirements.contains(SummoningRequirement.DISCARD_1_CARD)) {
                ArrayList<Integer> indexesOfSpecialSummoningMonsters = aiBoardAnalysis.getIndexesOfSpecialSummoningMonsters();
                indexesOfSpecialSummoningMonsters.add(i);
            }
    ...
    private void updateInformationAboutSpellCardsInTheseArrayLists(int i, SpellCard spellCard, AIBoardAnalysis boardAnalysis) {
        if (spellCard != null) {
            ...
            if (normalSpellCardEffect.contains(NormalSpellCardEffect.BOTH_PLAYERS_DISCARD_ALL_CARDS_FROM_HAND_AND_DRAW_CARDS_EQUAL_TO_THROWN_AWAY_CARDS)) {
                ArrayList<Integer> indexesOfCardRemovalSpells = boardAnalysis.getIndexesOfCardRemovalSpells();
                indexesOfCardRemovalSpells.add(i);
            }
