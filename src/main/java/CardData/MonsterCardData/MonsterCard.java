...
import CardEffects.MonsterEffectEnums.*;
...
public class MonsterCard extends Card {
    ...
    public MonsterCard(int attackPower, int defensePower, int level, MonsterCardAttribute attribute, MonsterCardFamily monsterCardFamily, MonsterCardValue monsterCardValue, String cardName, String cardDescription, CardPosition cardPosition, int numberOfAllowedUsages) {
        super(cardName, CardType.MONSTER, cardDescription, cardPosition, numberOfAllowedUsages);
    ...
    public static int giveATKDEFConsideringEffects(String string, CardLocation cardLocation, int index) {
        ...
        if (equipSpellEffects.contains(EquipSpellEffect.EQUIPPED_MONSTER_GAIN_800_ATK_DEF_FOR_EACH_FACE_UP_MONSTER_OWNER_CONTROLS)) {
            ...
            if (rowOfCardLocation.equals(RowOfCardLocation.ALLY_MONSTER_ZONE)) {
                ...
                for (int i = 0; i < 5; i++) {
                    CardLocation cardLocationOfAllyMonster = new CardLocation(RowOfCardLocation.OPPONENT_MONSTER_ZONE, i);
                    Card card = duelBoard.getCardByCardLocation(cardLocationOfAllyMonster);
                    if (card != null) {
                        MonsterCard allyMonsterCard = (MonsterCard) card;
                        if (allyMonsterCard.getCardPosition().equals(CardPosition.FACE_UP_ATTACK_POSITION)) {
