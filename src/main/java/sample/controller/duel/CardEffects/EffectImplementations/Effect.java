...
import sample.controller.duel.CardEffects.MonsterEffectEnums.BeingAttackedEffect;
import sample.controller.duel.CardEffects.MonsterEffectEnums.SummoningRequirement;
import sample.controller.duel.CardEffects.SpellEffectEnums.*;
import sample.controller.duel.CardEffects.SpellEffectEnums.LogicalActivationRequirement;
import sample.controller.duel.CardEffects.SpellEffectEnums.UserReplyForActivation;
import sample.controller.duel.CardEffects.TrapEffectEnums.*;
...
public class Effect {
    ...
    public static MessagesFromEffectToControllers arePreparationsCompleteForActivatingSpellTrapCard(CardLocation cardLocation, int index) {
        ...
        if (Card.isCardASpell(card)) {
            ...
            TrapCard trapCard = (TrapCard) card;
            ArrayList<sample.controller.duel.CardEffects.TrapEffectEnums.LogicalActivationRequirement> logicalActivationRequirements = trapCard.getLogicalActivationRequirements();
            if (logicalActivationRequirements.contains(sample.controller.duel.CardEffects.TrapEffectEnums.LogicalActivationRequirement.OWNER_MUST_HAVE_AT_LEAST_ONE_CARD_IN_HAND)) {
                if (!logicalActivationRequirementSpellOwnerMustHaveAtLeast1CardInHand(duelBoard, fakeTurn)) {
                    return MessagesFromEffectToControllers.PREPARATIONS_FOR_ACTIVATION_OF_THIS_TRAP_ARE_NOT_COMPLETE;
                }
            }
            if (logicalActivationRequirements.contains(sample.controller.duel.CardEffects.TrapEffectEnums.LogicalActivationRequirement.OPPONENT_MUST_HAVE_AT_LEAST_ONE_CARD_IN_HAND)) {
                if (!logicalActivationRequirementSpellOwnerMustHaveAtLeast1CardInHand(duelBoard, 3 - fakeTurn)) {
                    return MessagesFromEffectToControllers.PREPARATIONS_FOR_ACTIVATION_OF_THIS_TRAP_ARE_NOT_COMPLETE;
                }
            }
            if (logicalActivationRequirements.contains(sample.controller.duel.CardEffects.TrapEffectEnums.LogicalActivationRequirement.OWNER_MUST_HAVE_AT_LEAST_ONE_MONSTER_IN_THEIR_GY)) {
            ...
            if (logicalActivationRequirements.contains(sample.controller.duel.CardEffects.TrapEffectEnums.LogicalActivationRequirement.NORMAL_SUMMONED_MONSTER_MUST_HAVE_AT_LEAST_1000_ATK)) {
                if (!logicalActivationRequirementTrapSummonedMonsterMustHaveAtLeast1000ATK(index)) {
                    return MessagesFromEffectToControllers.PREPARATIONS_FOR_ACTIVATION_OF_THIS_TRAP_ARE_NOT_COMPLETE;
                }
            }
            if (logicalActivationRequirements.contains(sample.controller.duel.CardEffects.TrapEffectEnums.LogicalActivationRequirement.FLIP_SUMMONED_MONSTER_MUST_HAVE_AT_LEAST_1000_ATK)) {
                if (!logicalActivationRequirementTrapSummonedMonsterMustHaveAtLeast1000ATK(index)) {
                    return MessagesFromEffectToControllers.PREPARATIONS_FOR_ACTIVATION_OF_THIS_TRAP_ARE_NOT_COMPLETE;
                }
            }
            if (logicalActivationRequirements.contains(sample.controller.duel.CardEffects.TrapEffectEnums.LogicalActivationRequirement.MONSTER_IS_SUMMONED_OR_SPELL_TRAP_MONSTER_EFFECT_INCLUDING_SPECIAL_SUMMONING)) {
    ...
    public static ArrayList<String> inputsNeededForActivatingSpellTrapCard(CardLocation cardLocation, int index) {
        ...
        if (Card.isCardASpell(card)) {
            ...
            TrapCard trapCard = (TrapCard) card;
            ArrayList<sample.controller.duel.CardEffects.TrapEffectEnums.UserReplyForActivation> userReplyForActivations = trapCard.getUserReplyForActivations();
            if (userReplyForActivations.contains(sample.controller.duel.CardEffects.TrapEffectEnums.UserReplyForActivation.DISCARD_1_CARD)) {
                output.add("please choose one card from your hand to discard.\nSimple enter select command");
            }
            if (userReplyForActivations.contains(sample.controller.duel.CardEffects.TrapEffectEnums.UserReplyForActivation.CHOOSE_ONE_MONSTER_FROM_OWNERS_GRAVEYARD_TO_SPECIAL_SUMMON)) {
                output.add("show graveyard\nplease choose one monster from your own graveyard\nSimply enter select command");
            }
            if (userReplyForActivations.contains(sample.controller.duel.CardEffects.TrapEffectEnums.UserReplyForActivation.ENTER_NAME_OF_A_CARD)) {
